...
地心地固坐标系（Earth Center Earth Fixed, ECEF）和图1地理坐标系公用相同的坐标原点和坐标轴，但是使用笛卡尔坐标而非角度来描述点的位置。
...
在这一步的坐标系转换中，经度的计算比较简单，即$$tan \lambda = \frac{Y}{X}$$。维度的计算较为复杂，如下图所示（摘自[wikipedia](https://en.wikipedia.org/wiki/Geographic_coordinate_conversion)）。

![]({{site.baseurl}}/images/ECEF_2_geodetic.png)
**<center>图10. ECEF to GCS </center>**
